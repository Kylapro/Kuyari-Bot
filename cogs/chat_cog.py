...
class ChatCog(commands.Cog):
    ...
    async def on_ready(self) -> None:
        if client_id := self.bot.config.get("client_id"):
            logging.info(
    ...
    async def on_message(self, new_msg: discord.Message) -> None:
        ...
        if not is_dm and bot.user not in new_msg.mentions:
            bot.config = await asyncio.to_thread(get_config)
        ...
        while curr_msg is not None and len(messages) < max_messages:
            curr_node = bot.msg_nodes.setdefault(curr_msg.id, MsgNode())

            async with curr_node.lock:
                if curr_node.text is None:
                    cleaned_content = curr_msg.content.removeprefix(bot.user.mention).lstrip()
                    ...
                    curr_node.role = "user" if curr_msg.author.id == new_msg.author.id else "assistant"
                    curr_node.user_id = curr_msg.author.id

                    curr_node.has_bad_attachments = any(
                        att.size > max_text
                        for att in curr_msg.attachments
                        if att.content_type and att.content_type.startswith("text")
                    ) or len(curr_node.images) > max_images

                    curr_node.fetch_parent_failed = (
                        curr_msg.reference is None or curr_msg.reference.resolved is None
                    )
                    curr_node.parent_msg = curr_msg.reference.resolved if curr_msg.reference else None

                if curr_node.has_bad_attachments:
                    user_warnings.add("Attachments were truncated.")
                if curr_node.fetch_parent_failed:
                    user_warnings.add("Could not fetch parent message.")
                if curr_node.text:
                    content: list[Any] = []
                    if curr_node.text:
                    ...
                    if accept_usernames and curr_node.role == "user":
                        content.append({"type": "text", "text": f"username:{curr_msg.author.display_name}"})
                    messages.append({"role": curr_node.role, "content": content})
            ...
            async with new_msg.channel.typing():
                async for chunk in await openai_client.chat.completions.create(**kwargs):
                    ...
                    if not use_plain_responses:
                        ...
                        ) and datetime.now().timestamp() - bot.last_task_time >= EDIT_DELAY_SECONDS
                        msg_split_incoming = (
                        ...
                        is_good_finish = (
                            finish_reason is not None and finish_reason.lower() in ("stop", "end_turn")
                        )
                        ...
                        if start_next_msg or ready_to_edit or is_final_edit:
                            ...
                            if start_next_msg:
                                ...
                                bot.msg_nodes[response_msg.id] = MsgNode(parent_msg=new_msg)
        ...
        if (num_nodes := len(bot.msg_nodes)) > MAX_MESSAGE_NODES:
            for msg_id in sorted(bot.msg_nodes.keys())[: num_nodes - MAX_MESSAGE_NODES]:
                async with bot.msg_nodes.setdefault(msg_id, MsgNode()).lock:
